...
import _pickle as pickle #_pickle is cpickle
from math import e
import pdb
...
def load_model():
    ...
    modeldir='/home/xprize/work/final_models/'
    for i in range(5):
        try:
...
def predict(start_date, end_date, path_to_ips_file, output_file_path):
    ...
    data_path = '/home/xprize/work/data/adjusted_data.csv'
    ...
    monthly_temperature = pd.read_csv('/home/xprize/work/data/temp_1991_2016.csv')
    ...
    for g in ips_df.GeoID.unique():
        ...
        geo_pred_df['PredictedDailyNewCases_lower'] = np.array(geo_preds_lower[:len(geo_pred_df)])*(population/100000) #Adjust for population
        geo_pred_df['PredictedDailyNewCases_upper'] = np.array(geo_preds_upper[:len(geo_pred_df)])*(population/100000) #Adjust for population
        #Deaths
    ...
    pred_df.drop(columns={'GeoID'}).to_csv(output_file_path, index=False)
