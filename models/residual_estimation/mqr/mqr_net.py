...
parser.add_argument('--X_train', nargs=1, type= str,
                  default=sys.stdin, help = 'Path to train data.')
...
parser.add_argument('--residuals', nargs=1, type= str,
                  default=sys.stdin, help = 'Path to residuals for true values of train data.')
...
#Append pred to X_train
X_train = np.append(X_train,np.array([train_preds[days_ahead-1,:]]).T,axis=1)
#Get redisuals for day
y_train = residuals[days_ahead-1,:]
#Seed
...
for tr_idx, val_idx in kf.split(X_train):
    ...
    #Test the net
    random_selection = np.random.choice(X_train.shape[0],100)
    test(net, X_train[random_selection],y_train[random_selection])
    pdb.set_trace()
pdb.set_trace()
