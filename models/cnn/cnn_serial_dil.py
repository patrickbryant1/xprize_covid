...
def build_net():
    ...
    #Convolutions - wavenet like

    x = L.Conv1D(filters = filters, kernel_size = kernel_size, dilation_rate = 1, padding ="same")(x_in)
    x = L.BatchNormalization()(x)
    x = L.Activation('relu')(x)
    x = L.Conv1D(filters = filters, kernel_size = kernel_size, dilation_rate = 2, padding ="same")(x)
    x = L.BatchNormalization()(x)
    x = L.Activation('relu')(x)
    x = L.Conv1D(filters = filters, kernel_size = kernel_size, dilation_rate = 4, padding ="same")(x)
    x = L.BatchNormalization()(x)
    x = L.Activation('relu')(x)
    x = L.Conv1D(filters = filters, kernel_size = kernel_size, dilation_rate = 8, padding ="same")(x)
    x = L.BatchNormalization()(x)
    x = L.Activation('relu')(x)
